...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-400'>
        ...
        {
            token ? <div className='flex items-center gap-2 cursor-pointer group relative ' onClick={()=>setShowMenu(!showMenu)}>
        ...
        <div className={` ${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 bottom-0 top-0 z-20 overflow-hidden bg-white transition-all`}>
            ...
            <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
                <NavLink className={} onClick={()=>setShowMenu(false)} to='/'>Home</NavLink>
                <NavLink className={} onClick={()=>setShowMenu(false)} to='/doctors'>ALL Doctors</NavLink>
                <NavLink className={} onClick={()=>setShowMenu(false)} to='/about'>ABOUT</NavLink>
                <NavLink className={} onClick={()=>setShowMenu(false)} to='/contact'>CONTACT</NavLink>
            </ul>
        </div>
        </div>
    </div>
